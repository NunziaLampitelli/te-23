...
export default function Main() {
  ...
  useEffect(() => {
    ...
    if (storedCart) {
      try {
        ...
        if (Array.isArray(parsedCart) && parsedCart.every(item => item && item.id && item.title && item.price)) { //It verifies that the data saved in the array are objects that follow the Product arrey attributes
  ...
  const toggleTheme = () => { // function to change theme from the previous one (if dark -> light, if light -> dark)
    setIsDarkMode((prevMode) => !prevMode);
  };


  const products: Product[] = [ //arrey of objects that follow the Product interfaces attributes
    {
      id: 1,
      title: "Hamburger",
      image: PicOne,
  ...
  const addToCart = (product: Product) => { // function to add an item in the cart, it uses product as parameter and it has to be an object that follows the Product interface attributes
  ...
  const removeFromCart = (productId: number) => { // function to remove an item from the cart, productId becomes our parameter and it should be a number
    setCart((prevCart) => { // here we update the cart value, starting from the existing one 
      ...
      if (indexToRemove !== -1) { // if the index found is not -1 (so it exists and has a value) then
  ...
  const clearCart = () => { // localStorage function to remove all the saved data and show an empty arrey of the cart
    setCart([]);
  };
      ...
      <div className="main-container">
        <section className="products-container">
          {products.map((product) => (
            <article
              key={product.id}
              className={`product-card ${isDarkMode ? "dark-card" : "light-card"}`}
            >
